...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        evaluated_operands = scheme_eval(expressions.rest.first, env)
        env.define(expressions.first, evaluated_operands)
        ...
        position = formals
        while (position is not nil):
            if (not scheme_symbolp(position.first)):
                raise SchemeError('bad formals')
            position = position.rest

        env.define(function_name, LambdaProcedure(formals, body, env))
...
def do_and_form(expressions, env):
    ...
    for i in range(len(expressions)):
        if expressions.rest is nil:
            evaluated_expression = scheme_eval(expressions.first, env, True)
        else:
            evaluated_expression = scheme_eval(expressions.first, env, False)

        if not is_scheme_true(evaluated_expression):
            return False
...
def do_or_form(expressions, env):
    ...
    for i in range(len(expressions)):
        ...
        if is_scheme_true(evaluated_expression):
            return evaluated_expression
